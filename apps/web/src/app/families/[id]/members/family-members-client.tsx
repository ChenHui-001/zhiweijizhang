...
export default function FamilyMembersClient({ params }: FamilyMembersClientProps) {
  ...
  if (familyId === 'placeholder') {
    return (
      <PageContainer title="成员管理" showBackButton onBackClick={() => router.push('/families')} activeNavItem="profile">
  ...
  if (isLoading && members.length === 0) {
    return (
      <PageContainer
        title="成员管理"
        showBackButton
        onBackClick={() => router.push(`/families/${familyId}`)}
      ...
      {userPermissions.canInvite && (
        <>
          <InvitationHistory
            ...
            onRefresh={async () => { if (token) await fetchInvitations(token); }}
          />
        </>
      )}
