...
export function VersionManager({
  ...
  const {
    isChecking,
    hasUpdate,
    latestVersion: rawLatestVersion,
    userStatus,
    error,
    checkVersion,
  ...
  // 将 latestVersion 的 platform 字段转换为联合类型
  const latestVersion = rawLatestVersion
    ? {
        ...rawLatestVersion,
        platform: rawLatestVersion.platform.toUpperCase() as 'WEB' | 'IOS' | 'ANDROID',
      }
    : undefined;
...
export function ManualVersionCheck({
  ...
  const [showUpdateModal, setShowUpdateModal] = useState(false);
  const { isChecking, hasUpdate, latestVersion: rawLatestVersion, setUserVersionStatus, checkVersion } =
    useVersionCheck();
