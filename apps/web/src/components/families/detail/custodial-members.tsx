...
export function CustodialMembers({ familyId, isAdmin }: CustodialMembersProps) {
  ...
  // 处理头像更新
  const handleAvatarUpdate = async (avatarData: { type: 'preset'; data: any } | { type: 'file'; data: File }) => {
    ...
    try {
      ...
      if (avatarData.type === 'preset') {
        // 预设头像，使用头像ID
        avatarValue = avatarData.data.id;
      } else {
        // 文件上传，需要先上传文件获取URL
        const formData = new FormData();
        formData.append('avatar', avatarData.data);
        const uploadResponse = await fetchApi(`/api/users/${selectedMember.userId}/avatar`, {
          method: 'POST',
          body: formData,
        });
      ...
      console.log('🖼️ 更新托管成员头像:', { userId: selectedMember.userId, avatar: avatarValue });
